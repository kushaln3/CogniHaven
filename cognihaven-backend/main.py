...
async def enroll_session(batch: TelemetryBatch, db: Session = Depends(get_db)):
    ...
    # Classify user speed based on flight_mean
    # Fast: < 150ms, Medium: 150-300ms, Slow: > 300ms
    flight_mean = features[2]
    if flight_mean < 150:
        classification = "Fast"
    elif flight_mean < 300:
...
# --- Real-Time Inference with Delta Profiling & Justification ---

@app.post("/telemetry-stream")
async def process_telemetry(batch: TelemetryBatch, db: Session = Depends(get_db)):
    user_id = SESSIONS.get(batch.session_id)
    if not user_id:
        raise HTTPException(status_code=401, detail="Invalid session")
    ...
    # 3. Contextual Rule Engine Layer (Telemetry Traps)
    ...
    # Log to Audit Table with Justification
    action_str = f"{batch.action}"
    if justifications:
        action_str += f" [{', '.join(justifications[:2])}]"
...
async def reset_biometrics(req: CreateUserRequest, db: Session = Depends(get_db)):
    ...
    # Log the reset
    new_log = AuditLog(user_id=user.id, action="biometrics_reset_by_admin", risk_score=0, status="allowed")
    db.add(new_log)
    db.commit()
